...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        perror("Usage: ./ann_train <number>.");  // 0 两个都训练，1-训练英语和数字，2-训练汉字
        return 0;
    }
    if (!strcmp(argv[1], "0")) {
        train();
...
void train_zh() {
    ...
    // 设置学习率
    // classifier->setBackpropWeightScale(0.001);  // 添加反向传播学习率参数（0.001）

    // 迭代算法的终止条件 迭代次数(CV_TERMCRIT_ITER)和误差最小值(CV_TERMCRIT_EPS)，一旦有一个达到条件就终止训练
    classifier->setTermCriteria(TermCriteria(TermCriteria::MAX_ITER, 100000, 0.000001));  // 迭代100000次或误差小于0.000001

    printf("【汉字】训练数据准备完成，开始训练\n");
...
void train() {
    ...
    layers.at<int>(1) = 128;  // 第一个隐藏层设置128个节点
    layers.at<int>(2) = 128;   // 第二个隐藏层设置128个节点
    layers.at<int>(3) = 34;   // 输出类别数
    classifier->setLayerSizes(layers);
